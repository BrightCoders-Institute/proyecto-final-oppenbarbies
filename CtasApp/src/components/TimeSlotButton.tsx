...
const TimeSlotButton: React.FC<TimeSlotButtonProps> = ({ slots }) => {
  ...
  const handleSlotSelection = (timeSlot: string) => {
    ...
    Alert.alert("Time Slot Selected", `You have selected the ${timeSlot} slot.`);
  };

  return (
    <View>
      <Button text="Select Time Slot" onPress={() => setModalVisible(true)} styleName='welcome' textStyleName='welcome'/>
